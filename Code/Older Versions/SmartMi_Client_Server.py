...
while True:
    ...
    if state != State.SPLASH:
        ...
        if event == "-CAPTURE-":
            ...
            ## Frame contours import
            contours = get_frame_contour(face)
            if len(contours) == 0:
                window["-OUTPUT-"].update('No glasses frame detected.')
                continue
            leftContour = np.reshape(np.array(contours[0]), (contours[0].shape[0], 2))
            rightContour = np.reshape(np.array(contours[1]), (contours[1].shape[0], 2))
